...
| *camel.health.config.allow-core-thread-time-out* | Sets whether to allow core threads to timeout |  | Boolean
...
| *camel.health.config.failure-threshold* | Set the number of failures before reporting the service as un-healthy. |  | Integer
...
| *camel.health.config.keep-alive-time* | Sets the keep alive time for inactive threads |  | Long
| *camel.health.config.max-pool-size* | Sets the maximum pool size |  | Integer
| *camel.health.config.max-queue-size* | Sets the maximum number of tasks in the work queue. Use -1 or an unbounded queue |  | Integer
...
| *camel.threadpool.time-unit* | Sets the default time unit used for keeping alive time |  | TimeUnit
