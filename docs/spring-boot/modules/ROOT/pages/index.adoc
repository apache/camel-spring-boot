...
NOTE: It is recommended to import Camel Spring Boot BOM before Spring Boot BOM to ensure Camel dependencies are using supported JARs from the release. However, you can flip the order, and let Spring Boot BOM be first. However, you may encounter some dependency misalignments in some rare situations.

Next, add the xref:spring-boot.adoc[Camel Spring Boot starter] to start up the xref:manual::camelcontext.adoc[Camel Context].
...
Also add any xref:list.adoc[component starters] your Spring Boot application requires. For example this adds the xref:starters/jms.adoc[auto-configuration starter] for the xref:components::jms-component.adoc[JMS component].
