...
public class SpringBootAutoConfigurationMojo extends AbstractSpringBootGenerator {
     ...
     * main artifacts. Make sure it is not the case before enabling this property.
     */
    ...
    private void executeModels(JarFile componentJar, Map<String, Supplier<String>> files)
    ...
    private void createEipModelConfigurationSource(String packageName, EipModel model, String propertiesPrefix,
            boolean generatedNestedConfig) throws MojoFailureException {
        final int pos = model.getJavaType().lastIndexOf(".");
        final String commonName = model.getJavaType().substring(pos + 1)
                + (generatedNestedConfig ? "Common" : "Properties");
        final String configName = model.getJavaType().substring(pos + 1)
                + (generatedNestedConfig ? "Properties" : null);
        ...
        for (EipOptionModel option : model.getOptions()) {
            ...
            if ("id".equalsIgnoreCase(name) || "parent".equalsIgnoreCase(name)
                    || "camelContext".equalsIgnoreCase(name)) {
        ...
        if (generatedNestedConfig) {
            ...
            configClass.addField().setName("enabled").setType(boolean.class).setPrivate().setLiteralInitializer("true")
                    .getJavaDoc().setFullText("Enable the component");
            configClass.addField().setName("configurations")
                    .setType(loadType("java.util.Map<java.lang.String, " + packageName + "." + commonName + ">"))
                    .setPrivate().setLiteralInitializer("new HashMap<>()").getJavaDoc()
                    .setFullText("Define additional configuration definitions");
    ...
    private void createRestConfigurationSource(String packageName, EipModel model, String propertiesPrefix)
    ...
    private void createRestModuleAutoConfigurationSource(String packageName, EipModel model)
            throws MojoFailureException {
        ...
        javaClass.addAnnotation(AutoConfiguration.class).setStringValue("afterName",
                "org.apache.camel.spring.boot.CamelAutoConfiguration");
        javaClass.addAnnotation(ConditionalOnBean.class).setStringValue("type",
                "org.apache.camel.spring.boot.CamelAutoConfiguration");
        javaClass.addAnnotation(ConditionalOnProperty.class).setStringValue("name", "camel.rest.enabled")
                .setLiteralValue("matchIfMissing", "true");
    ...
    private void executeComponents(JarFile componentJar, Map<String, Supplier<String>> jsonFiles)
            throws MojoFailureException {
        ...
        if (!componentNames.isEmpty()) {
            ...
            Map<String, List<ComponentModel>> grModels = allModels.stream()
                    .collect(Collectors.groupingBy(ComponentModel::getJavaType));
            for (String componentClass : grModels.keySet()) {
                ...
                // equivalent
                List<String> aliases = compModels.stream().map(ComponentModel::getScheme).sorted()
    ...
    private void beforeGenerateComponentSource(ComponentModel model) {
        if ("activemq".equals(model.getScheme()) || "amqp".equals(model.getScheme())
                || "stomp".equals(model.getScheme())) {
            // we want brokerURL to be brokerUrl so its generated with a nice name for spring boot (and some other
            // options too)
            model.getComponentOptions().stream().filter(o -> "brokerURL".equals(o.getName())).findFirst()
                    .ifPresent(o -> o.setName("brokerUrl"));
            model.getComponentOptions().stream().filter(o -> "useMessageIDAsCorrelationID".equals(o.getName()))
                    .findFirst().ifPresent(o -> o.setName("useMessageIdAsCorrelationId"));
            model.getComponentOptions().stream().filter(o -> "includeAllJMSXProperties".equals(o.getName())).findFirst()
                    .ifPresent(o -> o.setName("includeAllJmsxProperties"));
            model.getComponentOptions().stream().filter(o -> "includeSentJMSMessageID".equals(o.getName())).findFirst()
                    .ifPresent(o -> o.setName("includeSentJmsMessageId"));
        }
    }
    ...
    private void executeLanguages(JarFile componentJar, Map<String, Supplier<String>> jsonFiles)
            throws MojoFailureException {
        ...
        if (!languageNames.isEmpty()) {
            ...
            // Group the models by implementing classes
            Map<String, List<LanguageModel>> grModels = allModels.stream()
                    .collect(Collectors.groupingBy(LanguageModel::getJavaType));
            for (String languageClass : grModels.keySet()) {
                ...
                // equivalent
                List<String> aliases = dfModels.stream().map(LanguageModel::getName).sorted()
                        .collect(Collectors.toList());
    ...
    private void createComponentConfigurationSource(String packageName, ComponentModel model,
            String overrideComponentName) throws MojoFailureException {
        int pos = model.getJavaType().lastIndexOf(".");
        ...
        bogus.getField().getJavaDoc().setText(
                "Whether to enable auto configuration of the " + scheme + " component. This is enabled by default.");
    ...
    private boolean isComplexTypeOrDuration(ComponentOptionModel option) {
        ...
        if (option.getJavaType().startsWith("java.util.Map") || option.getJavaType().startsWith("java.util.List")
    ...
    private JavaClass readJavaType(String type) {
        if (!type.startsWith("java.lang.") && (!type.contains("<") || !type.contains(">"))) {
            ...
            try {
                ...
                URL url = clazz != null
                        ? getProjectClassLoader().getResource(clazz.getName().replace('.', '/') + ".class") : null;
                ...
                JavaClass nestedType = new JavaClass(getProjectClassLoader()).setPackage(clazz.getPackage().getName())
                        .setName(clazz.getSimpleName()).setEnum(clazz.isEnum()).setClass(!clazz.isInterface())
                        .setAbstract((clazz.getModifiers() & Modifier.ABSTRACT) != 0)
                        .setStatic((clazz.getModifiers() & Modifier.STATIC) != 0)
                        .extendSuperType(clazz.getGenericSuperclass() != null
                                ? new GenericType(clazz.getGenericSuperclass()).toString() : null);
                ...
                List<java.lang.reflect.Method> allSetters = publicMethods.stream()
                        ...
                        .filter(m -> m.getName().matches("set[A-Z][a-zA-Z0-9]*")).collect(Collectors.toList());
                List<java.lang.reflect.Method> allGetters = publicMethods.stream()
                        .filter(m -> m.getReturnType() != void.class).filter(m -> m.getParameterCount() == 0)
                        .filter(m -> m.getName().matches("(get|is)[A-Z][a-zA-Z0-9]*")).collect(Collectors.toList());
                allSetters.stream()
                        .sorted(Comparator.<java.lang.reflect.Method> comparingInt(
                                m -> getSetterPosition(sourceCode, m)).thenComparing(java.lang.reflect.Method::getName))
                        .map(m -> StringUtils.uncapitalize(m.getName().substring(3))).forEach(fn -> {
                            Class<?> ft;
                            Type wft;
                            boolean isBoolean;
                            java.lang.reflect.Field field = Stream.of(clazz.getDeclaredFields())
                                    .filter(f -> f.getName().equals(fn)).findAny().orElse(null);
                            List<java.lang.reflect.Method> setters = allSetters.stream()
                                    .filter(m -> m.getName().equals("set" + StringUtils.capitalize(fn)))
                                    .collect(Collectors.toList());
                            List<java.lang.reflect.Method> getters = allGetters.stream()
                                    .filter(m -> m.getName().equals("get" + StringUtils.capitalize(fn))
                                            || m.getName().equals("is" + StringUtils.capitalize(fn)))
                                    .collect(Collectors.toList());
                            ...
                            if (setters.size() == 1) {
                                ...
                                accessor = allGetters.stream()
                                        .filter(m -> m.getName().equals("get" + StringUtils.capitalize(fn))
                                                || isBoolean && m.getName().equals("is" + StringUtils.capitalize(fn)))
                                        .filter(m -> PRIMITIVE_CLASSES.getOrDefault(m.getReturnType(),
                                                m.getReturnType()) == wft)
                                        .findAny().orElse(null);
                            } else if (field != null) {
                                ft = field.getType();
                                ...
                                mutator = allSetters.stream()
                                        .filter(m -> m.getName().equals("set" + StringUtils.capitalize(fn)))
                                        .filter(m -> PRIMITIVE_CLASSES.getOrDefault(m.getParameterTypes()[0],
                                                m.getParameterTypes()[0]) == wft)
                                        .findAny().orElse(null);
                                accessor = allGetters.stream()
                                        .filter(m -> m.getName().equals("get" + StringUtils.capitalize(fn))
                                                || isBoolean && m.getName().equals("is" + StringUtils.capitalize(fn)))
                                ...
                                wft = PRIMITIVE_CLASSES.getOrDefault(ft, ft);
                                mutator = setters.stream().filter(m -> PRIMITIVE_CLASSES
                                        .getOrDefault(m.getParameterTypes()[0], m.getParameterTypes()[0]) == wft)
                                        .findAny().orElse(null);
                                accessor = getters.stream().filter(m -> PRIMITIVE_CLASSES
                                        .getOrDefault(m.getReturnType(), m.getReturnType()) == wft).findAny()
                                        .orElse(null);
                            }
                            if (mutator == null) {
            ...
            if (idx > 0) {
                ...
                if (idx > 0) {
                    ...
                    List<String> lines = Stream.of(sourceCode.split("\n")).map(String::trim)
                            .map(s -> s.startsWith("*") ? s.substring(1) : s).map(String::trim)
                    ...
                    sourceCode = lines.subList(0, lastLine).stream().map(s -> s.replaceAll("  ", " ")).map(String::trim)
    ...
    private void addAnnotation(Function<Class<? extends java.lang.annotation.Annotation>, Annotation> creator,
    ...
    private static boolean skipComponentOption(ComponentModel model, ComponentOptionModel option) {
        if ("netty-http".equals(model.getScheme())) {
            ...
            if (name.equals("textline") || name.equals("delimiter") || name.equals("autoAppendDelimiter")
                    || name.equals("decoderMaxLineLength") || name.equals("encoding")
                    || name.equals("allowDefaultCodec") || name.equals("udpConnectionlessSending")
                    || name.equals("networkInterface") || name.equals("clientMode") || name.equals("reconnect")
                    || name.equals("reconnectInterval") || name.equals("useByteBuf") || name.equals("udpByteArrayCodec")
                    || name.equals("broadcast")) {
                return true;
            }
        }
        return false;
    ...
    private void createDataFormatConfigurationSource(String packageName, DataFormatModel model,
        ...
        // javadoc as description in its metadata
        Property bogus = javaClass.addProperty("java.lang.Boolean", "enabled");
        bogus.getField().getJavaDoc().setText("Whether to enable auto configuration of the " + model.getName()
        ...
        for (LanguageOptionModel option : model.getOptions()) {
            ...
            if ("bean".equals(model.getName())) {
                ...
                if ("token".equals(option.getName()) || "endToken".equals(option.getName())
                        || "inheritNamespaceTagName".equals(option.getName()) || "headerName".equals(option.getName())
                        || "regex".equals(option.getName()) || "xml".equals(option.getName())
    ...
    private void createComponentAutoConfigurationSource(String packageName, ComponentModel model, String overrideName,
        ...
        if (complexOptions) {
            ...
            javaClass.addAnnotation(AutoConfiguration.class)
                    .setLiteralValue("after", "{CamelAutoConfiguration.class, " + cn + ".class}");
        } else {
            javaClass.addAnnotation(AutoConfiguration.class).setLiteralValue("after", "CamelAutoConfiguration.class");
        }
    ...
    private void createDataFormatAutoConfigurationSource(String packageName, DataFormatModel model, String overrideName,
        ...
        final String name = model.getJavaType().substring(model.getJavaType().lastIndexOf(".") + 1)
                .replace("DataFormat", "DataFormatAutoConfiguration");
        final String configurationName = name.replace("DataFormatAutoConfiguration", "DataFormatConfiguration");
        final String dataformatName = camelCaseToDash(overrideName != null ? overrideName : model.getName())
                .toLowerCase(Locale.US);
        final Class<?> configClass = generateDummyClass(packageName + "." + configurationName);
        final JavaClass javaClass = new JavaClass(getProjectClassLoader());
    ...
    private void createDataFormatSpringFactorySource(String packageName, DataFormatModel model)
            throws MojoFailureException {
        int pos = model.getJavaType().lastIndexOf(".");
        String name = model.getJavaType().substring(pos + 1);
    ...
    private void createLanguageSpringFactorySource(String packageName, LanguageModel model)
            throws MojoFailureException {
        int pos = model.getJavaType().lastIndexOf(".");
        String name = model.getJavaType().substring(pos + 1);
    ...
    private static String createLanguageBody(String shortJavaType, String name) {
        return new StringBuilder().append("return new LanguageCustomizer() {\n").append("    @Override\n")
                ...
                .append("        return HierarchicalPropertiesEvaluator.evaluate(\n")
                .append("                applicationContext,\n")
                .append("                \"camel.language.customizer\",\n").append("                \"camel.language.")
                .append(name).append(".customizer\")\n").append("            && target instanceof ")
    ...
    private void findComponentNames(Set<String> componentNames) {
        componentJar.stream().filter(je -> !je.isDirectory()).map(ZipEntry::getName)
                ...
                .filter(s -> !s.startsWith(".") && !s.contains("/")).forEach(componentNames::add);
    }
    ...
    private void writeSourceIfChanged(JavaClass source, String fileName, boolean innerClassesLast)
            throws MojoFailureException {
