...
public class UnitTestCommand extends AbstractTestCommand implements Command {
    ...
    public UnitTestResult executeTest(final ITestConfig config, String component) throws Exception {
        ...
        LauncherDiscoveryRequest request = LauncherDiscoveryRequestBuilder.request()
                ...
                .filters(includeClassNamePatterns(config.getUnitTestInclusionPattern()),
                        excludeClassNamePatterns(config.getUnitTestExclusionPattern()), new IsAdmissableFilter(),
        ...
        if (nbTests > 0) {
            ...
            logger.info(config.getModuleName() + " unit tests. " + "Success: " + testSucceeded + " - Test Run: "
                    + result.getTestsStartedCount() + " - Failures: " + result.getTestsFailedCount()
            ...
            for (Failure f : result.getFailures()) {
                failureString
                        .append(f.getTestIdentifier().getDisplayName() + " - " + f.getException().getMessage() + "\n");
            ...
            if (!testSucceeded) {
                if (failureException != null) {
                    Assertions.fail(
                            "Some unit tests failed (" + result.getTestsFailedCount() + "/"
                                    + result.getTestsStartedCount() + ") : " + failureString.toString(),
                            failureException);
                } else {
                    Assertions.fail("Some unit tests failed (" + result.getTestsFailedCount() + "/"
                            + result.getTestsStartedCount() + ") : " + failureString.toString());
                }
            }
            ...
            if (expectedTests != null && expectedTests != result.getTestsStartedCount()) {
                Assertions.fail(
                        "Wrong number of tests: expected " + expectedTests + " found " + result.getTestsStartedCount());
            }
