...
public class KameletGlobalPropertiesTest {
    ...
    public void propertiesAreTakenFromRouteId() {
        ...
        assertThat(fluentTemplate.to("kamelet:setBody/test").request(String.class)).isEqualTo("from-route");
    }
    ...
    public void rawPropertyRefIsPropagated() {
        ...
        assertThat(context.getEndpoints().stream().filter(HttpEndpoint.class::isInstance).findFirst()
                .map(HttpEndpoint.class::cast)).get().hasFieldOrPropertyWithValue("endpointUri",
    ...
    protected RoutesBuilder createRouteBuilder() {
        ...
        return new RouteBuilder() {
            ...
            public void configure() {
                ...
                // template
                routeTemplate("http-send").templateParameter("proxyUsr").templateParameter("proxyPwd")
                        .from("kamelet:source").log("info")
                        .to("http://localhost:8080?proxyAuthUsername=RAW({{proxyUsr}})&proxyAuthPassword=RAW({{proxyPwd}})");

                // template
                routeTemplate("timer-source").templateParameter("period").templateParameter("message")
                        .from("timer:tick").setBody().constant("{{message}}").to("kamelet:sink");
