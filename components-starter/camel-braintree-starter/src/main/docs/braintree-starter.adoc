...
| *camel.component.braintree.access-token* | The access token granted by a merchant to another in order to process transactions on their behalf. Used in place of environment, merchant id, public key and private key fields. |  | String
...
| *camel.component.braintree.environment* | The environment Either SANDBOX or PRODUCTION |  | String
| *camel.component.braintree.http-log-level* | Set logging level for http calls, see java.util.logging.Level |  | String
| *camel.component.braintree.http-log-name* | Set log category to use to log http calls. | Braintree | String
| *camel.component.braintree.http-read-timeout* | Set read timeout for http calls. |  | Integer
...
| *camel.component.braintree.log-handler-enabled* | Sets whether to enable the BraintreeLogHandler. It may be desirable to set this to 'false' where an existing JUL - SLF4J logger bridge is on the classpath. This option can also be configured globally on the BraintreeComponent. | true | Boolean
| *camel.component.braintree.merchant-id* | The merchant id provided by Braintree. |  | String
| *camel.component.braintree.private-key* | The private key provided by Braintree. |  | String
| *camel.component.braintree.proxy-host* | The proxy host |  | String
| *camel.component.braintree.proxy-port* | The proxy port |  | Integer
| *camel.component.braintree.public-key* | The public key provided by Braintree. |  | String
|===
