...
| *camel.dataformat.zipfile.max-decompressed-size* | Set the maximum decompressed size of a zip file (in bytes). An IOException will be thrown if the decompressed size exceeds this amount. Set to -1 to disable setting a maximum decompressed size. By default there is no maximum size enforced. |  | Long
