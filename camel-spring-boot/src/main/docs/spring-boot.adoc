...
| *camel.springboot.inflight-repository-exchange-enabled* | Sets whether the inflight repository should track each inflight exchange. This is by default disabled as there is a very slight performance overhead when enabled. | false | Boolean
