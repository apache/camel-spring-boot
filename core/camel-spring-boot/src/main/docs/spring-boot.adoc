...
metrics backend are on the classpath. Add the `camel-ai-observability-starter` dependency to opt in to first-class
...
written to the Camel `PropertiesComponent` local properties as `camel.aiObservability.enabled`, matching Camel Main and
JBang. See the Camel xref:next@components:others:ai-observability.adoc[AI Observability] documentation for span
