...
Camel 4.23+ AI producers emit OpenTelemetry spans and Micrometer metrics when `camel-ai-observability` and a tracing or
metrics backend are on the classpath. Spring Boot exposes the global toggle as a first-class configuration property:
...
Relaxed binding also accepts `camel.aiObservability.enabled` and `camel.ai-observability.enabled`. The property is
written to the Camel `PropertiesComponent` as `camel.aiObservability.enabled`, matching Camel Main and JBang. See the
Camel xref:next@components:others:ai-observability.adoc[AI Observability] documentation for span attributes and metrics.
